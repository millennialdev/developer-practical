					...
					<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8'>
						...
						<div>
							...
							<div className='mt-6 flex flex-col gap-3'>
								...
								<a
									href={`mailto:${COMPANY.email}?subject=Project Inquiry - IronPeak Construction`}
									className='flex items-center gap-2 text-gray-400 text-sm min-h-[44px] hover:text-brand-red transition-colors'>
									<Mail size={16} />
									{COMPANY.email}
								</a>
						...
						<div>
							<h4 className='font-heading text-lg font-semibold mb-6 text-white!'>
								{
									FOOTER_SECTIONS.industries
										.title
								}
							</h4>
							<ul className='space-y-2.5'>
								{FOOTER_SECTIONS.industries.links.map(
									(link) => (
										<li key={link}>
						...
						<div>
							...
							<ul className='space-y-2.5'>
								{FOOTER_SECTIONS.usefulLinks.links.map(
									(link) => (
										<li key={link.href}>
											<a
												href={
						...
						<div>
							...
							<ul className='space-y-2.5'>
								{FOOTER_SECTIONS.coreServices.links.map(
									(link) => (
										<li key={link}>
											<span className='text-gray-400 text-sm hover:text-brand-red transition-colors cursor-pointer'>
												{link}
			...
			<div className='border-t border-gray-800 py-6'>
				<Container className='flex flex-col md:flex-row justify-between items-center gap-4'>
					<p className='text-gray-500 text-sm text-center md:text-left'>
						&copy; 2026 IronPeak Construction Group. All
						rights reserved.
					</p>
